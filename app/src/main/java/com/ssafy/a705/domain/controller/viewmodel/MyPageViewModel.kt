...
import com.ssafy.a705.domain.controller.service.MyPageService
import com.ssafy.a705.domain.controller.service.WithService
...
class MyPageViewModel @Inject constructor(
    ...
    fun loadMyComments(page: Int = _commentPage.value, size: Int = PAGE_SIZE) {
        viewModelScope.launch {
            ...
            _errorMessage.value = null
            try {
                val r = myPageService.getMyComments(page = page, size = size)
    ...
    fun loadMyProfile() {
        viewModelScope.launch {
            try {
                _profile.value = myPageService.getMyProfile()
            } catch (e: ApiException) {
    ...
    fun updateNickname(email: String, newNickname: String, onResult: (Boolean)->Unit = {}) {
        viewModelScope.launch {
            runCatching { myPageService.updateNickname(email, newNickname) }
                .onSuccess {
                    // 낙관적 갱신 or 재로딩 택1
