...
interface MyPageApi {
    ...
    ): Response<BaseResponse<MyBoardsPageResponse>>

    @GET("v1/mypage/comments")
    suspend fun getMyComments(
        @Query("page") page: Int? = 0,
        @Query("size") size: Int? = 10
    ): Response<BaseResponse<MyCommentsPageResponse>>
    ...
    suspend fun patchProfile(@Body body: PatchProfileRequest): Response<BaseResponse<Unit>>
